...
export default function CheckoutPage() {
        ...
        return (
            <div className="min-h-screen bg-xmart-bg">
                ...
                <div className="max-w-xl mx-auto px-4 py-20 text-center">
                    ...
                    <p className="text-zinc-400 mb-5 text-sm">{t('cart.emptyDesc')}</p>
    ...
    const validate = () => {
        ...
        if (!form.name.trim()) newErrors.name = language === 'en' ? 'Please enter full recipient name' : 'กรุณากรอกชื่อ-นามสกุล';
        if (!form.phone.trim() || !/^0\d{8,9}$/.test(form.phone.trim())) newErrors.phone = language === 'en' ? 'Please enter a valid phone number (starting with 0)' : 'กรุณากรอกเบอร์โทรให้ถูกต้อง (เริ่มด้วย 0)';
        if (!form.address.trim()) newErrors.address = language === 'en' ? 'Please enter delivery address' : 'กรุณากรอกที่อยู่';
        if (!form.province.trim()) newErrors.province = language === 'en' ? 'Please enter province' : 'กรุณากรอกจังหวัด';
        if (!form.district.trim()) newErrors.district = language === 'en' ? 'Please enter district' : 'กรุณากรอกเขต/อำเภอ';
        if (!form.subdistrict.trim()) newErrors.subdistrict = language === 'en' ? 'Please enter subdistrict' : 'กรุณากรอกแขวง/ตำบล';
        if (!form.postalCode.trim() || !/^\d{5}$/.test(form.postalCode)) newErrors.postalCode = language === 'en' ? 'Postal code must be 5 digits' : 'รหัสไปรษณีย์ต้องเป็น 5 หลัก';
        setErrors(newErrors);
    ...
    return (
        <div className="min-h-screen bg-xmart-bg pb-24">
            ...
            <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 grid md:grid-cols-[1fr_360px] gap-8 items-start">
                {/* Form */}
                <section>
                    <h1 className="text-2xl sm:text-3xl font-black mb-6 text-zinc-900">
                        {language === 'en' ? 'Delivery Details' : 'รายละเอียดการจัดส่ง'}
                    </h1>
                    <form id="checkout-form" onSubmit={handleSubmit} className="bg-white rounded-3xl p-5 sm:p-7 shadow-xs border border-zinc-200/80 space-y-4">
                        {field('name', language === 'en' ? 'Recipient Full Name *' : 'ชื่อ-นามสกุล ผู้รับ *', language === 'en' ? 'Full Name' : 'ชื่อ นามสกุล')}
                        {field('phone', language === 'en' ? 'Phone Number *' : 'เบอร์โทรศัพท์ *', '08XXXXXXXX', 'tel')}
                        <div>
                            <label className="block text-xs sm:text-sm font-bold text-zinc-800 mb-1.5">
                                {language === 'en' ? 'Delivery Address *' : 'ที่อยู่จัดส่ง *'}
                            </label>
                            <textarea value={form.address}
                                onChange={e => { setForm({ ...form, address: e.target.value }); setErrors({ ...errors, address: '' }); }}
                                rows={2} placeholder={language === 'en' ? 'House number, Soi, Road, Building or Landmark' : 'บ้านเลขที่ ซอย ถนน อาคาร หรือจุดสังเกต'}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                            {field('province', language === 'en' ? 'Province *' : 'จังหวัด *', language === 'en' ? 'Province' : 'จังหวัด')}
                            {field('district', language === 'en' ? 'District *' : 'เขต/อำเภอ *', language === 'en' ? 'District' : 'เขต/อำเภอ')}
                            {field('subdistrict', language === 'en' ? 'Subdistrict *' : 'แขวง/ตำบล *', language === 'en' ? 'Subdistrict' : 'แขวง/ตำบล')}
                            {field('postalCode', language === 'en' ? 'Postal Code *' : 'รหัสไปรษณีย์ *', language === 'en' ? '5 digits' : '5 หลัก', 'text', '[0-9]{5}')}
                        </div>

                        {/* Payment */}
                        <div className="pt-3 border-t border-zinc-100">
                            <h3 className="font-bold text-sm sm:text-base text-zinc-900 mb-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {(['CASH', 'PROMPTPAY'] as const).map(method => (
                                    <label key={method} className={`border-2 rounded-2xl p-4 flex flex-col items-center gap-2 cursor-pointer transition-all ${paymentMethod === method ? 'border-xmart-primary bg-blue-50/60 shadow-xs' : 'border-zinc-200 hover:bg-zinc-50'}`}>
                                        ...
                                        <span className="text-xs sm:text-sm font-bold text-center text-zinc-800">
                                            {method === 'CASH' ? (language === 'en' ? 'Cash on Delivery' : 'เงินสดปลายทาง') : 'QR PromptPay'}
                                        </span>
                                        {method === 'PROMPTPAY' && (
                                            <span className="text-[11px] text-xmart-primary font-bold bg-blue-100/70 px-2 py-0.5 rounded-full">
                                                {language === 'en' ? 'Instant Confirmation' : 'จ่ายก่อนได้สิทธิ์ก่อน'}
                                            </span>
                                        )}
                                    </label>
                                ))}
                ...
                <aside>
                    <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-xs border border-zinc-200/80 sticky top-24">
                        <h2 className="text-lg font-black text-zinc-900 mb-4">
                            {language === 'en' ? 'Order Summary' : 'สรุปออเดอร์'}
                        </h2>
                            ...
                            {items.map(item => (
                                <li key={item.product.id} className="py-2.5 flex justify-between items-center gap-2 text-sm">
                                    <div className="flex gap-2 items-center min-w-0">
                                        ...
                                        <span className="text-xs sm:text-sm font-medium text-zinc-800 line-clamp-1">
                                            {getProductName(item.product, language)}
                                        </span>
                                    </div>
                                    <span className="font-bold text-xs sm:text-sm text-zinc-900 shrink-0">฿{(item.product.price * item.quantity).toLocaleString()}</span>
                                </li>
                            ))}
                        </ul>
                        ...
                        <div className="py-3 border-t border-zinc-100 space-y-2 text-sm">
                            <div className="flex justify-between text-zinc-600">
                                <span>{language === 'en' ? 'Subtotal' : 'ยอดรวมสินค้า (Subtotal)'}</span>
                                <span className="font-semibold text-zinc-800">฿{getTotalPrice().toLocaleString()}</span>
                            </div>
                            <div className="flex justify-between text-emerald-600 font-bold">
                                <span>{language === 'en' ? 'Shipping Fee' : 'ค่าจัดส่ง (Shipping)'}</span>
                                <span>{language === 'en' ? 'Free (🚚 Free Shipping)' : '฿0 (ส่งฟรี 🚚)'}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between items-center py-3 border-t border-zinc-200 mb-5">
                            <span className="font-bold text-zinc-900 text-base">
                                {language === 'en' ? 'Grand Total' : 'ยอดสุทธิ (Grand Total)'}
                            </span>
                        ...
                        <button type="submit" form="checkout-form" disabled={loading}
                            ...
                            {loading
                                ? (language === 'en' ? 'Creating Order...' : 'กำลังสร้างออเดอร์...')
                                : paymentMethod === 'PROMPTPAY'
                                    ? (language === 'en' ? 'Proceed to Payment →' : 'ดำเนินการชำระเงิน →')
                                    : (language === 'en' ? 'Confirm Order (Cash)' : 'ยืนยันสั่งซื้อ (เงินสด)')}
                        </button>
                        <p className="text-center text-xs text-zinc-400 mt-3">
                            {paymentMethod === 'CASH'
                                ? (language === 'en' ? 'Pay upon delivery (Cash on Delivery)' : 'ชำระเงินเมื่อได้รับสินค้า (COD)')
                                : (language === 'en' ? 'Scan & Pay with PromptPay QR Code' : 'จ่ายผ่าน QR Code / PromptPay')}
                        </p>
                    </div>
                </aside>
            </main>
